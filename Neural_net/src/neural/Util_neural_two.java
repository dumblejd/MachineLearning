...
public class Util_neural_two 
{
	...
	String savepath="";
	public void train(String filepath_input,double percentage_train,int interation,int num_hidden,int...nums_node)
	{
	...
	{
		//Layer temp=null;
		int target=(int)(data[data.length-1]);
...
//		}
		...
		double temp=-1;
		for(int i=0;i<this.data_test.size();i++)
		{
			temp=data_test.get(i)[data_test.get(i).length-1];
			if(map_out.containsKey(temp))
			{
				
			}
			else
			{
				map_out.put(temp, temp);
			}
		}
		Layer output=new Layer();
		output.initial_layer(all_layer.get(all_layer.size()-1).value.length-1, map_out.size(), 2);
...
//		tempcount=size-tempcount;
	...
	public void accuracy_for_one_output(List<double[]> data)
	{
		...
		for(int i=0;i<data.size();i++)
		{
			...
			double max=-1;
			int maxindex=-1;
			for(int j=0;j<all_layer.get(all_layer.size()-1).value.length;j++)
			{
				//System.out.println(all_layer.get(all_layer.size()-1).value[j]);
				if(all_layer.get(all_layer.size()-1).value[j]>max)
				{
					max=all_layer.get(all_layer.size()-1).value[j];
					maxindex=j;
				}
			}
			//System.out.println("--");
			if(data.get(i)[data.get(i).length-1]==Double.valueOf(maxindex+1))
			{
				count++;
			}
			int mseindex=(int)data.get(i)[data.get(i).length-1]-1;
			double[]mins=new double[all_layer.get(all_layer.size()-1).value.length];
			...
			for(int w=0;w<all_layer.get(all_layer.size()-1).value.length;w++)
			{
				temp+=(mins[w]-all_layer.get(all_layer.size()-1).value[w])*(mins[w]-all_layer.get(all_layer.size()-1).value[w]);
			}
		}
		
		error=temp/data.size();
		System.out.println("MSE error:"+error);
		System.out.println("accuracy:"+count/((1.0)*data.size()));
	}
	public static void main(String[] args)
	{
//		Preprocessing p=new Preprocessing();
//		p.preprocess("/Users/dijin/eclipse-workspace/Neural_net/car.data", "/Users/dijin/Desktop/ML/homework作业/neural/standard_data.csv");
...
//		u.train("/Users/dijin/eclipse-workspace/Neural_net/iris.data.txt", 0.8, 1000, 2,2,2);
//		//u.print_net();
//		u.accuracy_for_one_output(u.data_train);
//		u.accuracy_for_one_output(u.data_test);
//		//u.test();
//		u.train("/Users/dijin/eclipse-workspace/Neural_net/iris.data.txt", 0.8, 1000, 3,3,4,5);
//		//u.print_net();
//		u.accuracy_for_one_output(u.data_train);
//		u.accuracy_for_one_output(u.data_test);
//		System.out.println("---------------");
//		u.train("/Users/dijin/eclipse-workspace/Neural_net/car.data.txt", 0.8, 1000, 2,2,2);
//		//u.print_net();
//		u.accuracy_for_one_output(u.data_train);
//		u.accuracy_for_one_output(u.data_test);
//		u.train("/Users/dijin/eclipse-workspace/Neural_net/car.data.txt", 0.8, 1000, 3,3,4,5);
//		//u.print_net();
//		u.accuracy_for_one_output(u.data_train);
//		u.accuracy_for_one_output(u.data_test);
//		System.out.println("---------------");
//		u.train("/Users/dijin/eclipse-workspace/Neural_net/adult.data.txt", 0.8, 1000, 2,2,2);
//		//u.print_net();
//		u.accuracy_for_one_output(u.data_train);
//		u.accuracy_for_one_output(u.data_test);
		u.train("/Users/dijin/eclipse-workspace/Neural_net/iris.data.txt", 0.8, 1000,4,3,3,3,3);
		//u.print_net();
		u.accuracy_for_one_output(u.data_train);
		u.accuracy_for_one_output(u.data_test);
		System.out.println("---------------");
